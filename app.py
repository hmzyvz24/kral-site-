...
# Ortak style (tüm sayfalarda aynı olsun diye)
common_style = """
<style>
    ...
    a { color: lime; font-size: 30px; text-decoration: none; margin: 20px; }
</style>
"""
...
def hakkinda():
    return f"""
    <html><head><title>Hakkımda</title>{common_style}</head>
    <body>
        <h1>Hakkımda</h1>
...
def iletisim():
    return f"""
    <html><head><title>İletişim</title>{common_style}</head>
    <body>
        <h1>İletişim</h1>
        ...
        <a href="https://instagram.com/h2m2za24">
            ...
            <br><span style="font-size: 35px;">h2m2za24</span>
        </a>
        <br><br><a href="/">Ana Sayfa</a>
    </body></html>
    """
...
def hesap_makinesi():
    return f"""
    <html><head><title>Hesap Makinesi</title>{common_style}</head>
    <body>
...
def flappy_bird():
    return f"""
    <html><head><title>Flappy Bird</title>{common_style}
        <style>
            canvas {{ background: skyblue; border: 5px solid lime; border-radius: 10px; }}
        </style>
    </head>
    <body>
...
# XOX, Yılan, Satranç sayfaları da aynı şekilde common_style ile başlayacak (önceki mesajlardaki gibi, aynı tasarım).
